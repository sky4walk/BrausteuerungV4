...
const char setup_html[] PROGMEM = R"rawliteral(
  ...
  <head></head>
    ...
    <h3> <a href="http://www.Brauverein-Schwabach.de/">Zwieselbrau.de</a></h3>
...
String processorTemp(const String& var){
  ...
  if(var == "ANAUS"){
    if ( TempWebServer::mSettings->getHeatState() ) {
      return String("On");
    } else {
      return String("Off");      
    }
...
String processorSetup(const String& var){
  //Serial.println(var);
  if(var == "OFFINPUT"){
    return String(TempWebServer::mSettings->getPidKp());
  } 
  else if(var == "ONINPUT"){
    return String(TempWebServer::mSettings->getPidKi());
  }
...
void setLevelGet(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam(PARAM_ON_INPUT)) {
      ...
      TempWebServer::mSettings->setPidKp(inputMessage.toDouble());
  ...
  if (request->hasParam(PARAM_OFF_INPUT)) {
      ...
      CONSOLELN(inputMessage);     
      TempWebServer::mSettings->setPidKi(inputMessage.toDouble());
      TempWebServer::mSettings->setShouldSave(true);
  }
